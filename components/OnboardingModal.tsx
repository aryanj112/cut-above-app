    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <View className="flex-1 bg-black/40 justify-center p-5">
        <View className="bg-white rounded-2xl p-5">
          <Text className="text-xl font-bold mb-3">Finish setting up your account</Text>

          <Text className="text-xs text-gray-500 mb-1">Preferred name</Text>
          <TextInput
            value={name}
          ...
          <DateTimePickerModal
            ...
            onConfirm={(d) => { setBirthday(toISODate(d)); setShowPicker(false); }}
            onCancel={() => setShowPicker(false)}
          />

          <View className="flex-row justify-end gap-2 mt-2">
            <TouchableOpacity onPress={onClose} className="bg-gray-200 px-4 py-2 rounded-xl">
              <Text className="font-semibold">Skip</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={save} disabled={saving} className="bg-gray-900 px-4 py-2 rounded-xl">
